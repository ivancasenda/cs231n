...
def _normalization_backward(dout, cache, axis):
    normalized, gamma, x_minus_mean, mean, var, inv_std, std, x = cache
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    if (H + 2 * pad - HH) % stride != 0:
        raise ValueError("Invalid combination of input height, pool height, and stride")
    if (W + 2 * pad - WW) % stride != 0:
        raise ValueError("Invalid combination of input width, pool width, adn stride")
        ...
        for f in range(F):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    out[n, f, h_out, w_out] = np.sum(x_pad[n, :, start_h:end_h, start_w:end_w] * w[f]) + b[f]
        ...
        for f in range(F):
            ...
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    dx_pad[n, :, start_h:end_h, start_w:end_w] += dout[n, f, h_out, w_out] * w[f]
                    dw[f] += dout[n, f, h_out, w_out] * x_pad[n, :, start_h:end_h, start_w:end_w]
        ...
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    out[n, c, h_out, w_out] = np.max(x[n, c, start_h:end_h, start_w:end_w])
        ...
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    ...
                    x_region = x[n, c, start_h:end_h, start_w:end_w]
                    max_x, max_y = np.unravel_index(np.argmax(x_region), x_region.shape)

                    dx[n, c, start_h:end_h, start_w:end_w][max_x][max_y] += dout[n, c, h_out, w_out]
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    if C % G != 0:
        raise ValueError("Incompatible number of groups to split into!")
    split_size = C // G
    ...
    for g in range(G):
        start_c = g * split_size
        end_c = start_c + split_size
        
        out[:, start_c:end_c], cache[f'g{g}'] = layernorm_forward(x_reshaped[:, start_c:end_c], gamma_reshaped[start_c:end_c], beta_reshaped[start_c:end_c], gn_param)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    for g in range(G):
        ...
        dx[:, start_c:end_c], dgamma[start_c:end_c], dbeta[start_c:end_c] = layernorm_backward(dout_reshaped[:, start_c:end_c], cache[f'g{g}'])
