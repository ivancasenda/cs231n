...
def tv_loss(img, tv_weight):
    ...
    _, H, W, C = img.shape
    vertical_variation = tf.math.reduce_sum(tf.math.square(img[0, 1:H, :, :] - img[0, 0:H-1, :, :]))
    horizontal_variation = tf.math.reduce_sum(tf.math.square(img[0, :, 1:W, :] - img[0, :, 0:W-1, :]))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        layer = style_layers[i]
        feats_gram = gram_matrix(feats[layer])
        target_gram = style_targets[i]
        style_loss += style_weights[i] * tf.math.reduce_sum(tf.math.square(feats_gram - target_gram))
...
def gram_matrix(features, normalize=True):
    ...
    G = tf.reshape(features, [H*W, C])
    G = tf.matmul(G, G, transpose_a=True)

    if normalize:
        G = tf.math.divide(G, H*W*C)
...
def content_loss(content_weight, content_current, content_original):
    ...
    return content_weight * tf.math.reduce_sum(tf.math.square(content_current - content_original))
